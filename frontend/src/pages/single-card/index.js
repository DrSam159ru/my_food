...
const SingleCard = ({ loadItem, updateOrders }) => {
  ...
  const showErrorNotification = (shortLink) => {
      ...
      .then(({ "short-link": shortLink }) => {
        ...
        if (canUseClipboardApi) {
          copyWithClipboardApi()
            ...
            .catch(() => {
              // Если Clipboard API не сработал (например, Safari) —
              // пробуем фолбэк и, если не получится, показываем ссылку текстом
              try {
                copyWithFallback();
                showSuccessNotification();
              } catch (e) {
                showErrorNotification(shortLink);
              }
            });
        } else {
          // Небезопасный контекст (http-домен) или старый браузер:
          // сразу используем фолбэк
          try {
            copyWithFallback();
            showSuccessNotification();
          } catch (e) {
